...
def trigger_alert():
    ...
    if not request.args.get("api_key") or request.args.get("api_key") != os.environ.get(
        "API_KEY", ""
    ):
        return "Invalid API key"
    gif_url = request.args.get(
        "gif", "https://media0.giphy.com/media/vKHKDIdvxvN7vTAEOM/giphy.gif"
    )
    audio_url = unquote_plus(
        request.args.get(
            "audio", "https://www.myinstants.com/media/sounds/mrbitcoin.mp3"
        )
    )
    ...
    borderWidth = request.args.get(
        "borderWidth", "2px"
    )  # Default to 1px if not provided
    color = request.args.get("color", "gold")
    duration = int(
        request.args.get("duration", 10000)
    ...
    if request.method == "POST":
        ...
        if "amount" in data:
            ...
            if option == 0:
                ...
                audio_url = unquote_plus(
                    "https://www.myinstants.com/media/sounds/cwisnotsatoshi.mp3"
                )
                width = "300px"
            else:
                gif_url = "https://media1.giphy.com/media/l49JMVDvP8D38LHwI/giphy.gif"
                audio_url = unquote_plus(
                    "https://www.myinstants.com/media/sounds/mrbitcoin.mp3"
                )
                width = "300px"
    ...
    alert_data = {
        ...
        "tts": (
            ""
            if tts == ""
            else f"https://api.streamelements.com/kappa/v2/speech?voice=Vitoria&text={tts}"
        ),
        "width": width,
        "height": height,
